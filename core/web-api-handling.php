	...
	protected static function validate_request() {
		...
		if (
			...
			Prompt_Logging::add_error(
				...
				__( 'Rejected an inbound request with an invalid timestamp. Could be bot activity.', 'Prompt_Core' ),
				$_REQUEST
			);
			return false;
		}
		...
		if ( empty( $_REQUEST['token'] ) ) {
			Prompt_Logging::add_error(
				...
				__( 'Rejected an inbound request with an invalid token. Could be bot activity.', 'Prompt_Core' ),
				$_REQUEST
			);
			return false;
		}
		...
		if ( !isset( $_REQUEST['signature'] ) or strlen( $_REQUEST['signature'] ) != 64 ) {
			Prompt_Logging::add_error(
				...
				__( 'Rejected an inbound request with an invalid signature. Could be bot activity.', 'Prompt_Core' ),
				$_REQUEST
			);
			return false;
		}
